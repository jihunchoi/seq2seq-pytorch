...
class Beam(object):
    def __init__(self, size, n_best, pad_id, bos_id, eos_id,
                 device=-1, vocab=None, global_scorer=None):
        self.size = size
        ...
        self.next_ys = [torch.LongTensor(size).fill_(pad_id)]
        self.next_ys[0][0] = bos_id
        self.vocab = vocab

        if device > -1:
            self.scores = self.scores.cuda(device)
            self.next_ys = [y.cuda(device) for y in self.next_ys]
    ...
    def advance(self, word_lk, attn_out):
        """
        ...
        * `attn_out`- attention at the last step
        ...
        # best_scores_d is flattened beam x word array, so calculate which
